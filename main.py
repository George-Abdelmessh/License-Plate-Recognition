...
if len(detection) == 0:
    ...
    cv.putText(car, text, (20, 40), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 3)
    cv.imshow('car', car)
    cv.waitKey(0)
else:
    cv.drawContours(car, [plate_cnt], -1, (0, 255, 0), 3)
    text = f'{detection[0][1]} {detection[0][2] * 100: .2f}%'
    cv.putText(car, text, (x, y-20), cv.FONT_HERSHEY_SIMPLEX, 0.75,(0, 255, 0), 3)
    cv.imshow('car', car)
    cv.waitKey(0)
